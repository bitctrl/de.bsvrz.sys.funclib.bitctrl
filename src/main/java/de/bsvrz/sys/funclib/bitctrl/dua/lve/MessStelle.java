...
public class MessStelle extends AbstractSystemObjekt {
	 ...
	 */
	private static Map<SystemObject, MessStelle> sysObjMsObjMap = new HashMap<SystemObject, MessStelle>();
	...
	protected MessStelle(final SystemObject msObjekt)
		...
		final ObjectSet mqMengeAbfahrten = konfigObjekt
				.getNonMutableSet("Abfahrten"); //$NON-NLS-1$
		for (final SystemObject mqObj : mqMengeAbfahrten.getElements()) {
			if (mqObj.isValid()) {
				...
				if (mqa != null) {
					...
					Debug.getLogger()
					.warning("Abfahrt " + mqObj + " an " + msObjekt + //$NON-NLS-1$//$NON-NLS-2$
							" konnte nicht identifiziert werden"); //$NON-NLS-1$
				}
			}
		}

		final ObjectSet mqMengeZufahrten = konfigObjekt
				.getNonMutableSet("Zufahrten"); //$NON-NLS-1$
		for (final SystemObject mqObj : mqMengeZufahrten.getElements()) {
			if (mqObj.isValid()) {
				...
				if (mqa != null) {
					...
					Debug.getLogger()
					.warning("Zufahrt " + mqObj + " an " + msObjekt + //$NON-NLS-1$//$NON-NLS-2$
							" konnte nicht identifiziert werden"); //$NON-NLS-1$
				}
			}
		}
		...
		final Data eigenschaften = msObjekt
				.getConfigurationData(atgEigenschaften);

		if (eigenschaften == null) {
			Debug.getLogger()
			.warning("\"atg.messStelle\" von Messstelle " + msObjekt + //$NON-NLS-1$
					" konnten nicht ausgelesen werden"); //$NON-NLS-1$
		} else {
			if (eigenschaften.getReferenceValue("Prüfling") != null) { //$NON-NLS-1$
				pruefling = eigenschaften.getReferenceValue("Prüfling") //$NON-NLS-1$
						.getSystemObject();
			}
		}
	}
	...
	protected static void initialisiere(final ClientDavInterface dav1)
		...
		for (final SystemObject msObj : sDav.getDataModel()
				.getType(DUAKonstanten.TYP_MESS_STELLE).getElements()) {
			if (msObj.isValid()) {
