...
public class UmfeldDatenSensorDatum {
	...
	public UmfeldDatenSensorDatum(final ResultData resultat) {
		...
		wert.setWert(datum.getItem(datenArt.getName()).getUnscaledValue("Wert") //$NON-NLS-1$
				.longValue());
	...
	public final ResultData getVeraendertesOriginalDatum() {
		...
		if (copy) {
			resultat = new ResultData(originalDatum.getObject(),
	...
	public final GanzZahl getGueteIndex() {
		...
		gueteIndex.setWert(datum.getItem(datenArt.getName()).getItem("Güte")
	...
	public final int getGueteVerfahren() {
		final int gueteVerfahren = datum.getItem(datenArt.getName())
				.getItem("Güte").getUnscaledValue("Verfahren").intValue(); //$NON-NLS-1$
	...
	public final long getT() {
		return datum.getTimeValue("T").getMillis(); //$NON-NLS-1$
	}
	...
	public final void setT(final long t) {
		erstelleKopie();
		datum.getTimeValue("T").setMillis(t); //$NON-NLS-1$
	}
	...
	public final int getStatusErfassungNichtErfasst() {
		return datum.getItem(datenArt.getName()).getItem("Status")
	...
	public final void setStatusErfassungNichtErfasst(
			final int statusErfassungNichtErfasst) {
		erstelleKopie();
		datum.getItem(datenArt.getName()).getItem("Status").getItem("Erfassung")
	...
	public final void setGueteIndex(final long guete) {
		...
		datum.getItem(datenArt.getName()).getItem("Güte")
	...
	public final void setStatusMessWertErsetzungImplausibel(
			final int statusMessWertErsetzungImplausibel) {
		erstelleKopie();
		datum.getItem(datenArt.getName()).getItem("Status")
	...
	public final void setStatusMessWertErsetzungInterpoliert(
			final int statusMessWertErsetzungInterpoliert) {
		erstelleKopie();
		datum.getItem(datenArt.getName()).getItem("Status")
	...
	public final Data getDatum() {
		if (wert.isVeraendert()) {
			erstelleKopie();
			datum.getItem(datenArt.getName()).getUnscaledValue("Wert") //$NON-NLS-1$
			.set(wert.getWert());
		}
		return datum;
	}
	...
	public String toString() {
		...
		s += "\nDatum: " + datum; //$NON-NLS-1$
		s += "\nDatenzeit: " + new Date(getDatenZeit()); //$NON-NLS-1$
		s += "\nWert: " + wert; //$NON-NLS-1$
