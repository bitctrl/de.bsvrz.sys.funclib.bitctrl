...
public final class DUAUtensilien {
	...
	public static void sendeBetriebsmeldung(final ClientDavInterface dav,
			...
			final String nachricht) {
		if (konverter == null) {
			sendeBetriebsmeldung(dav, grade, objekt, nachricht);
		}
		final String id = konverter.konvertiere(
				new BetriebsmeldungDaten(objekt), null, new Object[0]);
		MessageSender.getInstance().sendMessage(id,
				MessageType.APPLICATION_DOMAIN, null, grade, objekt,
				MessageState.MESSAGE, new MessageCauser(dav.getLocalUser(),
						Constants.EMPTY_STRING, Constants.EMPTY_STRING),
				nachricht);
	}
