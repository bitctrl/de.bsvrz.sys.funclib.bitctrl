	...
	private static void ermittleErsatzUndNachbarFS() {
		...
		for (final MessQuerschnitt mq : MessQuerschnitt.getInstanzen()) {
			for (final FahrStreifen fs : mq.getFahrStreifen()) {
				...
				if (fs.getNachbarFahrStreifen() == null) {
					Debug.getLogger().warning("Für Fahrstreifen " + fs //$NON-NLS-1$
							+ " kann " + //$NON-NLS-1$
				...
				if (fs.getErsatzFahrStreifen() == null) {
					 ...
					 * Ersatzmessstelle
					 */
					final MessQuerschnittAllgemein ersatzQuerschnitt = mq
							.getErsatzMessquerSchnitt();
					if (ersatzQuerschnitt != null) {
						final List<FahrStreifen> ersatzFahstreifen = new ArrayList<FahrStreifen>();
						for (final FahrStreifen fsAnErsatzQuerschnitt : ersatzQuerschnitt
								.getFahrStreifen()) {
							if (fsAnErsatzQuerschnitt.getLage()
									.equals(fs.getLage())) {
						...
						if (ersatzFahstreifen.size() > 0) {
							if (ersatzFahstreifen.size() > 1) {
								Debug.getLogger()
								.warning("Für Fahrstreifen " + fs //$NON-NLS-1$
										+ " sind mehrere" + //$NON-NLS-1$
										" Ersatzfahrstreifen ermittelbar."
										+ " Wähle: "
										+ ersatzFahstreifen.get(0));
							}
							fs.setErsatzFahrStreifen(
									ersatzFahstreifen.get(0).getSystemObject());
						}
					}
				}
