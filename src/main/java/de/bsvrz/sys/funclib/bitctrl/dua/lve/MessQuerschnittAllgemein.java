...
public abstract class MessQuerschnittAllgemein extends AbstractSystemObjekt {
	...
	protected MessQuerschnittAllgemein(final ClientDavInterface dav,
		...
		if (mqaObjekt == null) {
			throw new NullPointerException(
					"Übergebenes allgemeines Messquerschnittobjekt ist <<null>>"); //$NON-NLS-1$
		}

		final AttributeGroup atgEigenschaften = dav.getDataModel()
				.getAttributeGroup(DUAKonstanten.ATG_MQ_ALLGEMEIN);
		final Data eigenschaften = mqaObjekt
				.getConfigurationData(atgEigenschaften);

		if (eigenschaften == null) {
			Debug.getLogger().warning(
					"\"atg.messQuerschnittAllgemein\" von allgemeinem Messquerschnittobjekt " //$NON-NLS-1$
					+ mqaObjekt + " konnten nicht ausgelesen werden");
		} else {
			typ = MessQuerschnittTyp.getZustand(
					eigenschaften.getUnscaledValue("Typ").intValue()); //$NON-NLS-1$
			if (eigenschaften
					.getReferenceValue("ErsatzMessQuerschnitt") != null) { //$NON-NLS-1$
				ersatzQuerschnittObj = eigenschaften
						.getReferenceValue("ErsatzMessQuerschnitt") //$NON-NLS-1$
						.getSystemObject();
			}
		}
	}
	...
	public final MessQuerschnittAllgemein getErsatzMessquerSchnitt() {
		return getInstanz(ersatzQuerschnittObj);
	}
