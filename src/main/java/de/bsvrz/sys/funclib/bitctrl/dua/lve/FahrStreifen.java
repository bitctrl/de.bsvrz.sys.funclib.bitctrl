...
public class FahrStreifen extends AbstractSystemObjekt {
	...
	protected FahrStreifen(final SystemObject fsObjekt)
		...
		final Data eigenschaften = fsObjekt
				.getConfigurationData(atgEigenschaften);

		if (eigenschaften == null) {
			Debug.getLogger()
			.error("\"atg.fahrStreifen\" von Fahrstreifenobjekt " //$NON-NLS-1$
					+ fsObjekt + " konnten nicht ausgelesen werden");
		} else {
			lage = FahrStreifenLage.getZustand(
					eigenschaften.getUnscaledValue("Lage").intValue()); //$NON-NLS-1$
			if (eigenschaften.getReferenceValue("ErsatzFahrStreifen") != null) { //$NON-NLS-1$
				ersatzFahrstreifenObj = eigenschaften
						.getReferenceValue("ErsatzFahrStreifen") //$NON-NLS-1$
						.getSystemObject();
			}
		}
	}
	...
	protected static final void initialisiere(final ClientDavInterface dav1)
		...
		for (final SystemObject fsObjekt : sDav.getDataModel()
				.getType(DUAKonstanten.TYP_FAHRSTREIFEN).getElements()) {
			if (fsObjekt.isValid()) {
	...
	protected static final void initialisiere(final ClientDavInterface dav1,
		...
		for (final SystemObject fsObjekt : sDav.getDataModel()
				.getType(DUAKonstanten.TYP_FAHRSTREIFEN).getElements()) {
			if (fsObjekt.isValid()
					&& DUAUtensilien.isObjektInKBsEnthalten(fsObjekt, kbs)) {
				sysObjFsObjMap.put(fsObjekt, new FahrStreifen(fsObjekt));
			}
		}
	}
