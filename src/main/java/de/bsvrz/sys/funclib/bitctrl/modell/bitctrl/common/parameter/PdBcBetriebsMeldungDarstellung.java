	...
	public void setDaten(final ResultData result) {
		...
		if (result.hasData()) {
			...
			for (int i = 0; i < darstellung.getLength(); ++i) {
				...
				d.setKlasse(MeldungsKlasse.getMeldungsKlasse(
						item.getUnscaledValue("Klasse").intValue()));
				d.setVordergrundfarbe(
						item.getUnscaledValue("Vordergrundfarbe").longValue());
				d.setHintergrundfarbe(
						item.getUnscaledValue("Hintergrundfarbe").longValue());
			...
			for (int i = 0; i < anzuzeigendeSpalten.getLength(); ++i) {
				...
				datum.getAnzuzeigendeSpalten().add(BetriebsMeldungSpalte
						.valueOf(item.asUnscaledValue().intValue()));
			}
		}
