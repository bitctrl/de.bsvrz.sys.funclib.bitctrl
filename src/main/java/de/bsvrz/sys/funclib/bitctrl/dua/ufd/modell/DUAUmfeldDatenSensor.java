...
public class DUAUmfeldDatenSensor implements ClientReceiverInterface {
	 ...
	 */
	private static Map<SystemObject, DUAUmfeldDatenSensor> instanzen = new IdentityHashMap<SystemObject, DUAUmfeldDatenSensor>();
	...
	protected DUAUmfeldDatenSensor(final ClientDavInterface dav,
		...
		if (konfigDaten != null) {
			if (konfigDaten.getReferenceValue("Vorgänger") != null) { //$NON-NLS-1$
				vorgaenger = konfigDaten.getReferenceValue("Vorgänger") //$NON-NLS-1$
						.getSystemObject();
			}
