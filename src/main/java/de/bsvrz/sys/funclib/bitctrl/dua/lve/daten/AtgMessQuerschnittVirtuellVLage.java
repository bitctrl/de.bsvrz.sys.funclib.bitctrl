...
public class AtgMessQuerschnittVirtuellVLage implements MessQuerschnittAnteile {
	 ...
	 *             definiert ist.
	 */
	public AtgMessQuerschnittVirtuellVLage(final ClientDavInterface dav,
			final SystemObject objekt) throws KeineDatenException {
		final Data atgData = objekt.getConfigurationData(dav.getDataModel()
		...
		if (atgData
				.getReferenceValue("MessQuerschnittGeschwindigkeit") != null) {
			messQuerschnittGeschwindigkeit = atgData
					.getReferenceValue("MessQuerschnittGeschwindigkeit")
					.getSystemObject();
		}

		final List<AtlMessQuerSchnittBestandTeil> dummy = new ArrayList<AtlMessQuerSchnittBestandTeil>();
	...
	public static final class AtlMessQuerSchnittBestandTeil {
		 ...
		 */
		AtlMessQuerSchnittBestandTeil(final Data datum)
				throws InvalidArgumentException {
			...
			mqReferenz = datum.getReferenceValue("MessQuerschnittReferenz")
					.getSystemObject();
			final long anteilDummy = datum.getUnscaledValue("Anteil")
					.longValue();
			if (anteilDummy == 100) {
				anteil = 1.0;
