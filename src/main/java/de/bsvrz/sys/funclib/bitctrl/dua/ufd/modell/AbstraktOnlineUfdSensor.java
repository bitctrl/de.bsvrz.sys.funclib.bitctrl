...
implements ClientReceiverInterface {
	...
	protected void initialisiere(final ClientDavInterface dav,
			final SystemObject objekt1, final Aspect aspekt)
					throws UmfeldDatenSensorUnbekannteDatenartException {
		final UmfeldDatenArt datenArt = UmfeldDatenArt
				.getUmfeldDatenArtVon(objekt1);
		this.objekt = objekt1;
	 ...
	 */
	public final void addListener(final IOnlineUfdSensorListener<G> listener,
			final boolean informiereInitial) {
		if (informiereInitial) {
			synchronized (this) {
				if (this.listenerMenge.add(listener)
						&& (this.onlineWert != null)) {
					listener.aktualisiereDaten(this.onlineWert);
				}
			}
		} else {
