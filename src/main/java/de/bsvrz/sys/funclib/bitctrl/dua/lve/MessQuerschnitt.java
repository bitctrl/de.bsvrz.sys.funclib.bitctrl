...
public class MessQuerschnitt extends MessQuerschnittAllgemein {
	...
	protected MessQuerschnitt(final SystemObject mqObjekt) {
		...
		final ObjectSet fsMenge = konfigObjekt.getNonMutableSet("FahrStreifen"); //$NON-NLS-1$
		for (final SystemObject fsObj : fsMenge.getElements()) {
			if (fsObj.isValid()) {
				...
				if (fs != null) {
					...
					Debug.getLogger()
					.warning("Fahrstreifen " + fsObj + " an " + mqObjekt //$NON-NLS-1$//$NON-NLS-2$
	...
	protected static void initialisiere(final ClientDavInterface dav1) {
		...
		for (final SystemObject mqObjekt : sDav.getDataModel()
				.getType(DUAKonstanten.TYP_MQ).getElements()) {
			if (mqObjekt.isValid()) {
	...
	protected final FahrStreifen getNachbarVon(final FahrStreifen fs) {
		...
		if (fs.getLage() != null) {
			final FahrStreifenLage lageLinksVonHier = fs.getLage()
					.getLinksVonHier();
			final FahrStreifenLage lageRechtsVonHier = fs.getLage()
					.getRechtsVonHier();
