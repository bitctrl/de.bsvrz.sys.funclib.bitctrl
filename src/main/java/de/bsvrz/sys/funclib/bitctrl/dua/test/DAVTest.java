...
public final class DAVTest {
	 ...
	 */
	public static ClientDavInterface getDav(final String[] kommandoZeile)
			throws Exception {
	 ...
	 *             falls die Verbindung nicht hergestellt werden konnte
	 */
	public static ClientDavInterface newDav(final String[] kommandoZeile)
			throws Exception {
	...
	public static int[] getZufaelligeZahlen(final int anzahl) {
		...
		while (belegt < anzahl) {
			final int index = r.nextInt(anzahl);
			if (zahlen[index] == -1) {
				zahlen[index] = belegt++;
			}
		}
