...
implements IKontrollProzessListener<Long> {
	...
	private final SortedMap<Long, Collection<ObjektResultat>> kontrollZeitpunkte = Collections
			.synchronizedSortedMap(
					new TreeMap<Long, Collection<ObjektResultat>>());
	...
	public void initialisiere(final IVerwaltung dieVerwaltung)
		...
		kontrollProzess = new KontrollProzess<Long>();
			...
			for (final ResultData resultat : resultate) {
				if (resultat != null) {
					...
					if (getMaxZeitVerzug(resultat.getObject()) < 0) {
						 ...
						 */
						if (letzteEmpfangeneDatenZeitProObj
								.get(resultat.getObject()) < resultat
								.getDataTime()) {
						...
						if (resultat.getData() != null) {
							...
							final long kontrollZeitpunkt = getKontrollZeitpunktVon(
									resultat);
							if (kontrollZeitpunkt > 0) {
								Collection<ObjektResultat> kontrollObjekte = kontrollZeitpunkte
										.get(kontrollZeitpunkt);

								/**
								 * Kontrolldatum bestimmten
								 */
								final ObjektResultat neuesKontrollObjekt = new ObjektResultat(
										resultat);
								if (kontrollObjekte != null) {
									...
									kontrollObjekte.add(neuesKontrollObjekt);
									kontrollZeitpunkte.put(
											new Long(kontrollZeitpunkt),
											kontrollObjekte);
								}
							}
							...
							if (!kontrollZeitpunkte.isEmpty()) {
								fruehesterKontrollZeitpunkt = kontrollZeitpunkte
										.firstKey().longValue();
								...
								if (fruehesterKontrollZeitpunkt > 0) {
									...
									Debug.getLogger().warning(
											"Der momentan aktuellste Kontrollzeitpunkt ist <= 0"); //$NON-NLS-1$
								}
							} else {
								Debug.getLogger().warning(
										"Die Menge der Kontrollzeitpunkte ist leer"); //$NON-NLS-1$
							}
						}
					}
				}
	 ...
	 */
	private synchronized void bereinigeKontrollZeitpunkte(
		...
		final Long letzterErwarteterZeitpunkt = resultat.getDataTime()
				+ getTVon(resultat) + getMaxZeitVerzug(resultat.getObject());
	...
	public synchronized void trigger(final Long kontrollZeitpunkt) {
		final List<ResultData> zuSendendeAusfallDatenMenge = new ArrayList<ResultData>();

		final Collection<ObjektResultat> ausfallDaten = kontrollZeitpunkte
				.get(kontrollZeitpunkt);
		if (ausfallDaten != null) {
			for (final ObjektResultat ausfallDatum : ausfallDaten) {
				zuSendendeAusfallDatenMenge
				.add(getAusfallDatumVon(ausfallDatum.getDatum()));
			}
		} else {
			Debug.getLogger()
			.warning("Der Kontrollzeitpunkt " + //$NON-NLS-1$
					DUAKonstanten.ZEIT_FORMAT_GENAU
					.format(new Date(kontrollZeitpunkt))
		...
		if (zuSendendeAusfallDatenMenge.size() > 0) {
			aktualisiereDaten(
					zuSendendeAusfallDatenMenge.toArray(new ResultData[0]));
		}
	}
	...
	private long getKontrollZeitpunktVon(final ResultData empfangenesResultat) {
		...
		final long maxZeitVerzug = getMaxZeitVerzug(
