...
public class BmClient implements ClientReceiverInterface {
	 ...
	 * Beobachter.
	 */
	private final Set<IBmListener> listeners = Collections
	...
	protected BmClient(final ClientDavInterface dav) {
		...
		dav.subscribeReceiver(this,
				...
				datenBeschreibung, ReceiveOptions.normal(),
		...
		if (results != null) {
			for (final ResultData result : results) {
				if ((result != null) && (result.getData() != null)) {
					...
					final String text = result.getData()
							.getTextValue("MeldungsText").getText(); //$NON-NLS-1$
					SystemObject referenz = null;
					if ((result.getData().getReferenceArray("Referenz") != null) //$NON-NLS-1$
							&& (result.getData().getReferenceArray("Referenz") //$NON-NLS-1$
									.getLength() > 0)
							&& (result.getData().getReferenceArray("Referenz") //$NON-NLS-1$
									.getReferenceValue(0) != null)) {
						referenz = result.getData()
								.getReferenceArray("Referenz") //$NON-NLS-1$
								.getReferenceValue(0).getSystemObject();
					}
					synchronized (listeners) {
