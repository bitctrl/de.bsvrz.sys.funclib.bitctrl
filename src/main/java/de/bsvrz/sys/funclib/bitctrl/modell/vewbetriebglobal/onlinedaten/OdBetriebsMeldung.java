 ...
 * <p>
 * <em>Hinweis:</em> Dieser Datensatz ist mit bedacht zu verwenden, da er in der
 * Regel mit Hilfe von speziellen Klassen bzw. Methoden verwendet wird.
 *
 ...
 */
public class OdBetriebsMeldung
extends AbstractOnlineDatensatz<OdBetriebsMeldung.Daten> {
		...
		public boolean equals(final Object obj) {
			if (this == obj) {
				...
				equals &= ((meldungsTypZusatz == null)
						...
						|| ((meldungsTypZusatz != null) && meldungsTypZusatz
								.equals(o.meldungsTypZusatz));
	...
	protected Data konvertiere(final Daten datum) {
		...
		daten.getTextValue("ApplikationsKennung")
		.setText(datum.getApplikationsKennung());
		daten.getReferenceValue("ApplikationsTyp")
		.setSystemObject(datum.getApplikationsTyp().getSystemObject());
		daten.getTextValue("ID").setText(datum.getId());
		daten.getUnscaledValue("LaufendeNummer").set(datum.getLaufendeNummer());
		daten.getUnscaledValue("MeldungsKlasse")
		.set(datum.getMeldungsKlasse().getCode());
		daten.getUnscaledValue("Status")
		.set(datum.getMeldungsStatus().getCode());
		daten.getTextValue("MeldungsText").setText(datum.getMeldungsText());
		daten.getUnscaledValue("MeldungsTyp")
		.set(datum.getMeldungsTyp().getCode());
		daten.getTextValue("MeldungsTypZusatz")
		...
		if (datum.getReferenz() != null) {
			...
			array.setLength(1);
			array.getItem(0).asReferenceValue()
			.setSystemObject(datum.getReferenz().getSystemObject());
		}
	...
	public void setDaten(final ResultData result) {
		...
		if (result.hasData()) {
			...
			datum.setApplikationsId(
					daten.getUnscaledValue("ApplikationsID").longValue());
			datum.setApplikationsKennung(
					daten.getTextValue("ApplikationsKennung").getText());
			...
			datum.setLaufendeNummer(
					daten.getUnscaledValue("LaufendeNummer").longValue());
			datum.setMeldungsKlasse(MeldungsKlasse.getMeldungsKlasse(
					daten.getUnscaledValue("MeldungsKlasse").intValue()));
			datum.setMeldungsStatus(MeldungsStatus.getMeldungsStatus(
					daten.getUnscaledValue("Status").intValue()));
			datum.setMeldungsText(daten.getTextValue("MeldungsText").getText());
			datum.setMeldungsTyp(MeldungsTyp.getMeldungsTyp(
					daten.getUnscaledValue("MeldungsTyp").intValue()));
			datum.setMeldungsTypZusatz(
					daten.getTextValue("MeldungsTypZusatz").getText());

			final Array array = daten.getArray("Referenz");
			if (array.getLength() > 0) {
				datum.setReferenz(factory.getModellobjekt(
						array.getItem(0).asReferenceValue().getSystemObject()));
			}
