...
public final class Benutzerverwaltung {
		...
		public void datensatzAktualisiert(final DatensatzUpdateEvent event) {
			if (event.getDatum() instanceof AngemeldeteApplikationen.Daten) {
				...
				for (final AngemeldeteApplikationen.Daten.AngemeldeteApplikation app : CollectionUtilities
					...
					fireAngemeldet(app.getBenutzer(), app.getApplikation(),
							app.getSeit());
				}

				for (final AngemeldeteApplikationen.Daten.AngemeldeteApplikation app : CollectionUtilities
						.difference(letztesDatum, aktuellesDatum)) {
					fireAbgemeldet(app.getBenutzer(), app.getApplikation(),
							app.getSeit());
				}
	 ...
	 */
	public List<AngemeldeteApplikation> getAnmeldungen(
			final Benutzer benutzer) {
		final ObjektFactory factory = ObjektFactory.getInstanz();
		final Datenverteiler dav = factory.getDatenverteiler();
		final AngemeldeteApplikationen.Daten datum = dav
				.getOnlineDatensatz(AngemeldeteApplikationen.class)
				.abrufenDatum(
						AngemeldeteApplikationen.Aspekte.Standard.getAspekt());
	 ...
	 */
	public AngemeldeteApplikation getAnmeldungen(
			final Applikation applikation) {
		final ObjektFactory factory = ObjektFactory.getInstanz();
		final Datenverteiler dav = factory.getDatenverteiler();
		final AngemeldeteApplikationen.Daten datum = dav
				.getOnlineDatensatz(AngemeldeteApplikationen.class)
	...
	public List<Berechtigungsklasse> getBerechtigungsklasse() {
		...
		for (final SystemObjekt so : factory.bestimmeModellobjekte(
				SystemModellGlobalTypen.Berechtigungsklasse.getPid())) {
	 ...
	 * anderer Benuzter &auml;ndern und Autorisierungspassworte f&uuml;r andere
	 * Benuzter anlegen.<br>
	 * Da sich der Zustand des Admin-Flags derzeit nicht direkt &uuml;ber das
	 * DAV-API ermitteln l&auml;&szlig;t, hilft folgender Trick: Man versucht
	 * den Benuzter mit sich selbst zum Admin zu machen, klappt dies, so ist er
	 * bereits Admin gewesen, ansonsten ist er kein Admin.
	 *
	 * @param loginname
	 *            ein Nutzername
	 * @param passwort
	...
	public boolean isAdmin(final String loginname) {
		...
		final Berechtigungsklasse klasse = getBerechtigungsklasse(
	...
	public List<Rolle> getRolle() {
		...
		for (final SystemObjekt so : factory.bestimmeModellobjekte(
				SystemModellGlobalTypen.Rolle.getPid())) {
			rollen.add((Rolle) so);
		}
	...
	public List<Region> getRegion() {
		...
		for (final SystemObjekt so : factory.bestimmeModellobjekte(
				SystemModellGlobalTypen.Region.getPid())) {
	...
	public Benutzer anlegenBenutzer(final String adminLoginname,
		...
		try {
			benutzer = Benutzer.anlegen(pid, benutzerInfo.getLoginname(),
					benutzerInfo.getVorname(), benutzerInfo.getZweiterVorname(),
					benutzerInfo.getNachname(), benutzerInfo.getOrganisation(),
					benutzerInfo.getEmailAdresse());

			final ObjektFactory factory = ObjektFactory.getInstanz();
			final DataModel modell = factory.getVerbindung().getDataModel();
			final UserAdministration userAdmin = modell.getUserAdministration();
			userAdmin.createNewUser(adminLoginname, adminPasswort,
					benutzerInfo.getLoginname(), pid,
					benutzerInfo.getPasswort(), false,
					factory.getVerbindung().getLocalConfigurationAuthority()
					.getConfigurationArea().getPid());
		} catch (final ConfigurationChangeException ex) {
			throw new BenutzerChangeException(
					"Der Benutzer " + benutzerInfo.getLoginname()
	...
	public String checkPasswort(final String passwort, final Benutzer benutzer,
		...
		if ((benutzer != null) && passwortInfo.isVergleicheBenutzerdaten()) {
			final String loginname = benutzer.getName()
			...
			if (loginname.contains(
					lowerPasswort.subSequence(0, lowerPasswort.length() - 1))) {
				return "Das Passwort darf nicht im Loginnamen enthalten sein.";
			}
			if (lowerPasswort.contains(
					loginname.subSequence(0, loginname.length() - 1))) {
			...
			if (lowerPasswort
					.contains(nachname.subSequence(0, nachname.length() - 1))) {
				return "Das Passwort darf nicht den Nachnamen enthalten.";
			}

			if (lowerPasswort
					.contains(vorname.subSequence(0, vorname.length() - 1))) {
