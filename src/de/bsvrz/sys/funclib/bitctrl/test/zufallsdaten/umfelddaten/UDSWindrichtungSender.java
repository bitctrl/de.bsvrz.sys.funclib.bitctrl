...
import de.bsvrz.sys.funclib.bitctrl.modell.umfelddaten.UDSWindrichtungMesswert;
...
/**
 * Generiert Messwerte f&uuml;r Windrichtungssensoren.
 * 
 * @author BitCtrl Systems GmbH, Schumann
 * @version $Id$
 */
public class UDSWindrichtungSender extends AbstractUmfelddatensensorSender {

	/** Einziger Wert der hier generiert wird: die Windrichtung. */
	public static final String WERT = "WindRichtung";
	 ...
	 *            Windrichtungssensoren verwendet
	 */
	public UDSWindrichtungSender(ClientDavInterface verbindung,
		...
		for (SystemObject so : objekte) {
			if (so.isOfType(UmfelddatenModelTypen.UDS_WINDRICHTUNG.getPid())) {
				this.objekte.add(so);
			}
		}

		option = new MesswertOption("Windrichtung", 0, 359);
		option.addMesswertOptionListener(this);

		anzeige = new Anzeige("Windrichtung", "°");
		
		min = option.getMin();
		max = option.getMax();

		atg = modell.getAttributeGroup(UDSWindrichtungMesswert.WINDRICHTUNG
				.getAtgPID());
		asp = modell
				.getAspect(UDSWindrichtungMesswert.WINDRICHTUNG.getAspPID());
