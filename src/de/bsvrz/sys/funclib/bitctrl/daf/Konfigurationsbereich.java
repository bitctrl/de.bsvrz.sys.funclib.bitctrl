...
public class Konfigurationsbereich {
	 ...
	 */
	public static List<SystemObject> getObjekte(ConfigurationArea kb) {
		List<SystemObject> liste = new ArrayList<SystemObject>();
	...
	public static List<SystemObject> getObjekte(ConfigurationArea kb,
			String... typen) {
		List<SystemObject> liste = new ArrayList<SystemObject>();
		List<SystemObjectType> objekttypen = new ArrayList<SystemObjectType>();
		...
		if (typen == null)
			...
			liste.addAll(kb.getObjects(objekttypen, ObjectTimeSpecification
					.valid()));
		}
	...
	public static ConfigurationArea getStandardKonfigurationsBereich(
			ClientDavInterface verbindung) throws InvalidArgumentException {

		ConfigurationArea result = null;

		if (verbindung != null) {
			ConfigurationAuthority authority = verbindung
					.getLocalApplicationObject().getConfigurationArea()
					.getConfigurationAuthority();
			Data daten = authority.getConfigurationData(authority
					.getDataModel().getAttributeGroup(
							"atg.konfigurationsVerantwortlicherEigenschaften"));
			String bereichsPid = daten.getTextArray("defaultBereich")
					.getText(0);
			SystemObject objekt = authority.getDataModel().getObject(
					bereichsPid);
