...
extends AbstractSystemObjekt {
	 ...
	 * @param mqaObjekt Systemobjekt eines allgemeinen Messquerschnittes
	 */
	protected MessQuerschnittAllgemein(final ClientDavInterface dav,
		...
		if(mqaObjekt == null){
			throw new NullPointerException("Übergebenes allgemeines Messquerschnittobjekt ist <<null>>"); //$NON-NLS-1$
		}
		
		AttributeGroup atgEigenschaften = dav.getDataModel().getAttributeGroup(DUAKonstanten.ATG_MQ_ALLGEMEIN);
		Data eigenschaften = mqaObjekt.getConfigurationData(atgEigenschaften);
		
		if(eigenschaften == null){
			LOGGER.warning("Eigenschaften von allgemeinem Messquerschnittobjekt " + mqaObjekt + //$NON-NLS-1$
					" konnten nicht ausgelesen werden"); //$NON-NLS-1$
		}else{
			this.typ = MessQuerschnittTyp.getZustand(eigenschaften.getUnscaledValue("Typ").intValue()); //$NON-NLS-1$
			if(eigenschaften.getReferenceValue("ErsatzMessQuerschnitt") != null){ //$NON-NLS-1$
				this.ersatzQuerschnittObj = eigenschaften.getReferenceValue("ErsatzMessQuerschnitt").getSystemObject(); //$NON-NLS-1$
			}			
		}		
	}
	 ...
	 * dieser Klasse 
	 * 
	 * @param mqaObjekt ein MessQuerschnittAllgemein-Systemobjekt
	 ...
	 * definierten Fahrstreifen oder <code>null</code>, wenn hier keine Fahrstreifen
	 * definiert sind
	 */
