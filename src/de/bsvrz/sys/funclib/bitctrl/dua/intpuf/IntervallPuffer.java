...
public class IntervallPuffer<T extends IIntervallDatum<T>> {
		...
		for (long start : this.puffer.keySet()) {
			...
			if (intervall.getIntervallStart() < startIntervall
				...
				loeschListe.add(start);
					...
					&& intervall.getIntervallEnde() > startIntervall) {
				long restImIntervall = intervall.getIntervallEnde()
						- startIntervall;
				long restIntervallTeile = restImIntervall
						/ intervall.getGranularitaet();
				if (restIntervallTeile > 0) {
					intervall.setStart(intervall.getIntervallEnde()
							- (restIntervallTeile * intervall
									.getGranularitaet()));
				} else {
					loeschListe.add(start);
				}
	...
	protected class Intervall<T1 extends IIntervallDatum<T1>> extends
		...
		protected Intervall(final IIntervallPufferElement<T1> element)
			...
			this.granularitaet = this.getIntervallEnde()
					- this.getIntervallStart();
			if (this.granularitaet < 0) {
				throw new IllegalArgumentException(
						"Intervallende (" + this.getIntervallEnde() + //$NON-NLS-1$
								") liegt vor Intervallanfang ("
								+ this.getIntervallStart() + //$NON-NLS-1$
