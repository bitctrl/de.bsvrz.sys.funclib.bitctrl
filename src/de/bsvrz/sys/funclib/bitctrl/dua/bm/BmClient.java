...
public class BmClient implements ClientReceiverInterface {
	 ...
	 * Beobachter.
	 */
	private Set<IBmListener> listeners = Collections
			.synchronizedSet(new HashSet<IBmListener>());
	...
	protected BmClient(final ClientDavInterface dav) {
		DataDescription datenBeschreibung = new DataDescription(dav
				.getDataModel().getAttributeGroup("atg.betriebsMeldung"), //$NON-NLS-1$
				dav.getDataModel().getAspect("asp.information"), //$NON-NLS-1$
				(short) 0);
		dav.subscribeReceiver(this, dav.getDataModel()
				.getConfigurationAuthority(), datenBeschreibung, ReceiveOptions
		...
		if (results != null) {
			for (ResultData result : results) {
				if (result != null && result.getData() != null) {
					...
					final String text = result.getData().getTextValue(
							"MeldungsText").getText(); //$NON-NLS-1$
					SystemObject referenz = null;
					if (result.getData().getReferenceArray("Referenz") != null && //$NON-NLS-1$
							result.getData()
									.getReferenceArray("Referenz").getLength() > 0 && //$NON-NLS-1$
							result
									.getData()
									.getReferenceArray("Referenz").getReferenceValue(0) != null) { //$NON-NLS-1$
						referenz = result
								.getData()
								.getReferenceArray("Referenz").getReferenceValue(0).getSystemObject(); //$NON-NLS-1$
					}
					synchronized (this.listeners) {
						for (IBmListener listener : this.listeners) {
							listener.aktualisiereBetriebsMeldungen(referenz,
									zeit, text);
						}
					}
				}
