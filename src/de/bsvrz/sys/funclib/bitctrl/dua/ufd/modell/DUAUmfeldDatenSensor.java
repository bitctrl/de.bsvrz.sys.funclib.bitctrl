...
public class DUAUmfeldDatenSensor implements ClientReceiverInterface {
	 ...
	 */
	protected static Map<SystemObject, DUAUmfeldDatenSensor> instanzen = new HashMap<SystemObject, DUAUmfeldDatenSensor>();
	...
	protected DUAUmfeldDatenSensor(final ClientDavInterface dav,
		...
		Data konfigDaten = konfigObjekt.getConfigurationData(dav.getDataModel()
				.getAttributeGroup("atg.umfeldDatenSensor")); //$NON-NLS-1$

		if (konfigDaten != null) {
			if (konfigDaten.getReferenceValue("Vorgänger") != null) { //$NON-NLS-1$
				this.vorgaenger = konfigDaten
						.getReferenceValue("Vorgänger").getSystemObject(); //$NON-NLS-1$	
			}
			if (konfigDaten.getReferenceValue("Nachfolger") != null) { //$NON-NLS-1$
				this.nachfolger = konfigDaten
						.getReferenceValue("Nachfolger").getSystemObject(); //$NON-NLS-1$	
			}

			if (konfigDaten.getReferenceValue("ErsatzSensor") != null) { //$NON-NLS-1$
				this.ersatzSensor = konfigDaten.getReferenceValue(
						"ErsatzSensor").getSystemObject(); //$NON-NLS-1$	
			}
		...
		DataDescription parameterBeschreibung = new DataDescription(
				dav.getDataModel().getAttributeGroup(
						"atg.ufdsMessWertErsetzung"), //$NON-NLS-1$
		...
		if (resultate != null) {
			for (ResultData resultat : resultate) {
				if (resultat != null && resultat.getData() != null) {
					...
					this.maxZeitMessWertErsetzung = ufdsMessWertErsetzungData
							.getTimeValue("maxZeitMessWertErsetzung").getMillis(); //$NON-NLS-1$

					this.maxZeitMessWertFortschreibung = ufdsMessWertErsetzungData
							.getTimeValue("maxZeitMessWertFortschreibung").getMillis(); //$NON-NLS-1$
				}
			}
		}
	}
