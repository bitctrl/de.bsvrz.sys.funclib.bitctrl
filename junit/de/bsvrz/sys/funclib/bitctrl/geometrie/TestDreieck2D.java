...
/**
 * Testfall für die Klasse {@link de.bsvrz.sys.funclib.bitctrl.geometrie.Dreieck2D}
 * 
...
public class TestDreieck2D {
	...
	public final void testGetBounds2D() {
		Rectangle2D bounds = new Rectangle2D.Double(0, 0, 5, 5);
	...
	public final void testGetPathIteratorAffineTransform() {
		PathIterator path = dreieck.getPathIterator(null);

		for (int i = 0; i < 5; i++) {
			assertFalse(path.isDone());

			double coords[] = new double[6];
			int typ = path.currentSegment(coords);
